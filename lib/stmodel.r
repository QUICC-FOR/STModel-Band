...
stmodel_local <- function(params=params,inland=inland,outland=outland,x=x,y=y,a=a,b=b,clim_file=clim_file,writeStep=1,disturb=0,transProb=1,const_clim=TRUE){
    ...
    if(const_clim == FALSE){
    stmodel <- paste("./model/build/stmodel -x",  x ,"-y",  y ,"-a",  a, "-b",  b, " -c", clim_file, "-p", params,"-i",inland, "-t", writeStep, "-d", disturb,"-e",transProb, ">", outland, "2>/dev/null")
    }
